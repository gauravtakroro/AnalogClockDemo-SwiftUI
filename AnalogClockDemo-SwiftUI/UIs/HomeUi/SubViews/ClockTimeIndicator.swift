...
struct ClockTimeIndicator: View {
    ...
    enum `Type` {
        ...
        func angle(for time: ClockTimeModel) -> Angle {
            switch self {
        ...
        func baseRadiusFactor(for time: ClockTimeModel) -> CGFloat {
            switch self {
    ...
    let clockTime: ClockTimeModel
    
    var body: some View {
        GeometryReader { geometry in
            Path { path in
                let c = self.center(of: geometry.size)
                let baseRadius = 0.5 * self.type.baseRadiusFactor(for: self.clockTime) * min(geometry.size.width, geometry.size.height)
            ...
            .fill(self.type.color)
            .rotationEffect(self.type.angle(for: self.clockTime))
        }
    }
